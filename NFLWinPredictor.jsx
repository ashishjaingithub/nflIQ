  ...
  return (
    <div
      style={{
        ...
        borderRadius: 12,
        padding: 16,
        borderLeft: side === "left" ? `4px solid ${accent}` : `0.5px solid ${COLORS.border}`,
        borderRight: side === "right" ? `4px solid ${accent}` : `0.5px solid ${COLORS.border}`,
        minHeight: 220,
    ...
    >
      ...
      <div style={{ position: "relative" }}>
        <select
          ...
          style={{
            ...
            fontSize: 14, fontWeight: 500, appearance: "none",
      ...
      {team ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          >
            <div style={{ fontSize: 22, fontWeight: 700, color: team.primary, lineHeight: 1.1 }}>
              {team.name}
            </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            <Pill>{team.record.w}-{team.record.l}{team.record.t ? `-${team.record.t}` : ""}</Pill>
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "center", marginTop: 2 }}>
            <span
              style={{
                ...
                background: team.primary, border: `1px solid ${COLORS.border}`,
              }}
            />
            <span
              style={{
                width: 14, height: 14, borderRadius: 3,
                background: team.secondary, border: `1px solid ${COLORS.border}`,
              }}
            />
            <span style={{ fontSize: 11, color: COLORS.textMute, marginLeft: 4 }}>team colors</span>
          </div>
        </div>
      ) : (
        <div style={{ color: COLORS.textMute, fontSize: 13, fontStyle: "italic" }}>
...
function Pill({ children }) {
  return (
    <span
      style={{
        padding: "4px 10px", background: COLORS.raised,
...
// === ProbabilityBar ===
function ProbabilityBar({ probA, colorA, colorB }) {
...
// === FactorCard ===
function FactorCard({ factor, teamAName, teamBName, colorA, colorB, index }) {
  const Icon = ICON_MAP[factor.icon] || Sparkles;
  const isA = factor.winner === "A";
  const isB = factor.winner === "B";
  const isTie = factor.winner === "tie";
  const accent = isA ? COLORS.green : isB ? COLORS.red : COLORS.blue;
  const opacity = factor.magnitude === "major" ? 1 : factor.magnitude === "moderate" ? 0.6 : 0.3;
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        <div
          style={{
            ...
            background: COLORS.raised, borderRadius: 8,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 13, color: COLORS.textSec, marginTop: 2, lineHeight: 1.35 }}>
            {factor.description || factor.desc}
          </div>
        </div>
      </div>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
        <StatPill color={colorA} label={teamAName} value={factor.teamAValue} />
        <span style={{ color: COLORS.textMute, fontSize: 11, fontWeight: 600 }}>VS</span>
        <StatPill color={colorB} label={teamBName} value={factor.teamBValue} />
      </div>
      <div
        style={{
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {factors.map((f) => {
        ...
        const isA = f.delta > 0;
        const color = isA ? colorA : f.delta < 0 ? colorB : COLORS.blue;
        return (
          <div
            key={f.id}
            style={{
          ...
          >
            <div
              style={{
                color: COLORS.textSec, textAlign: "right",
...
function ScanningPanel({ teamA, teamB }) {
  const messages = [
    "Loading roster data...",
    "Analyzing quarterback stats...",
    "Calculating home field...",
    "Running matchup model...",
    "Finalizing prediction...",
  ];
  const [msgIdx, setMsgIdx] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setMsgIdx((i) => (i + 1) % messages.length), 200);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ color: COLORS.textMute, fontSize: 14, letterSpacing: 2 }}>VS</span>
      ...
      <div
        style={{
          ...
          fontFamily: "ui-monospace, monospace", minHeight: 18,
        }}
      >
        {messages[msgIdx]}
      </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 13, color: COLORS.textSec, marginTop: 2 }}>
              A STEM Fair Project — using real 2025 NFL data and math to predict football games
            </div>
          </div>
        </div>
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <div
          style={{
            display: "flex", alignItems: "center", justifyContent: "center",
            color: COLORS.textMute, fontSize: 12, fontWeight: 700, letterSpacing: 2,
            minWidth: 32,
          }}
        >
          VS
      ...
      >
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <LocChip
            ...
            disabled={!teamA}
          >
            {teamA ? `${teamA.name} home` : "Team A home"}
          </LocChip>
          <LocChip
          ...
          <LocChip
            ...
            disabled={!teamB}
          >
            {teamB ? `${teamB.name} home` : "Team B home"}
          </LocChip>
        </div>
      </div>
      ...
      {sameTeam && (
        <div
          style={{
            ...
            background: `${COLORS.red}15`, padding: "10px 14px",
            borderRadius: 8, border: `1px solid ${COLORS.red}55`,
          }}
        >
...
function RevealScreen({ teamA, teamB, matchup, revealedCount, animatedProb, currentProb }) {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        >
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: teamA.primary, lineHeight: 1.1 }}>
              {teamA.name}
            </div>
          </div>
          <div
          ...
          >
            <span style={{ fontSize: 36, fontWeight: 800, color: teamA.primary }}>
              {Math.round(probA)}%
            </span>
            <span style={{ color: COLORS.textMute, fontWeight: 700 }}>:</span>
            <span style={{ fontSize: 36, fontWeight: 800, color: teamB.primary }}>
              {Math.round(probB)}%
            </span>
          </div>
          <div style={{ textAlign: "right" }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: teamB.primary, lineHeight: 1.1 }}>
              {teamB.name}
            </div>
          </div>
        </div>
        <ProbabilityBar probA={probA} colorA={teamA.primary} colorB={teamB.primary} />
        <div
          style={{
        ...
        >
          ...
          <span style={{ color: COLORS.textMute, fontWeight: 500, letterSpacing: 0 }}>
            · {revealedCount} / {matchup.factors.length} factors
          </span>
        </div>
      </div>
      ...
      <div
        style={{
          ...
          padding: "0 4px", minHeight: 18,
        }}
      >
        {inProgress && visibleFactors.length > 0 && (
          <span>
            <BarChart3 size={14} style={{ verticalAlign: "-2px", marginRight: 6 }} />
            Analyzing factor {revealedCount} of {matchup.factors.length} —{" "}
            <span style={{ color: COLORS.text, fontWeight: 600 }}>
              {visibleFactors[visibleFactors.length - 1].label}
            </span>
            ...
          </span>
        )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {[...visibleFactors].reverse().map((f, i) => (
          <FactorCard
            key={f.id}
            factor={f}
    ...
    >
      <div
        style={{
          background: COLORS.surface,
          border: `0.5px solid ${COLORS.border}`,
          borderTop: `4px solid ${winner.primary}`,
          borderRadius: 16,
          padding: 24,
          display: "flex", flexDirection: "column",
      ...
      >
        ...
        <div
          style={{
            fontSize: 26, fontWeight: 700, color: winner.primary,
            textAlign: "center", lineHeight: 1.15,
          }}
        >
          {winner.name} wins this matchup
        </div>
        <div
          style={{
        ...
        <div
          style={{
            color: COLORS.textSec, fontSize: 13,
            textAlign: "center", maxWidth: 540, marginTop: 4, lineHeight: 1.45,
          }}
        >
          Based on 18 different factors from the 2025 NFL season, our model gives{" "}
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        >
          <Repeat size={16} /> Analyze another matchup
        </button>
        <button
        ...
        >
          <MessageSquare size={16} /> Explain this prediction
        </button>
      </div>
    </div>
  );
...
function SummaryCard({ label, value, sub, color }) {
  return (
    ...
    >
      ...
      <div
        style={{
          fontSize: 14, fontWeight: 700, color: color || COLORS.text,
          lineHeight: 1.2,
        }}
      >
        {value}
      ...
      {sub && (
        <div style={{ fontSize: 11, color: COLORS.textSec, fontWeight: 500 }}>{sub}</div>
      )}
    </div>
  );
}
  ...
  return (
    <div
      style={{
        ...
        background: COLORS.surface, border: `0.5px solid ${COLORS.border}`,
        borderRadius: 12, padding: 16,
        color: COLORS.textSec, fontSize: 13, lineHeight: 1.5,
      }}
    >
      <div
        style={{
          fontSize: 11, fontWeight: 600, letterSpacing: 1.5,
          color: COLORS.textMute, textTransform: "uppercase", marginBottom: 8,
        }}
      >
        How does this work?
      </div>
      We start at <strong style={{ color: COLORS.text }}>50–50</strong> (like a coin flip)
      and then look at <strong style={{ color: COLORS.text }}>18 different football facts</strong> —
      from quarterback play to home field to recent form. Each fact moves the prediction up
      or down a little. When we add them all up, we get our final prediction!
    </div>
  );
}
