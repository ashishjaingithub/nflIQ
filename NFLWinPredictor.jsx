...
import {
  ...
  Play, Sparkles, BarChart3, Repeat,
} from "lucide-react";
...
// Pick a foreground color for an arbitrary hex against `bg` that meets WCAG AA.
// Direction is chosen by bg luminance: dark bg => lighten; light bg => darken.
function fgFromHex(hex, bg = COLORS.surface) {
  if (!hex) return null;
  if (contrastRatio(hex, bg) >= 4.5) return hex;
  const adjust = relLum(bg) < 0.4 ? lighten : darken;
  let v = hex;
  for (let i = 0; i < 14 && contrastRatio(v, bg) < 4.5; i++) v = adjust(v, 0.07);
  return contrastRatio(v, bg) >= 4.5 ? v : null;
}
// Pick a foreground for a team: try primary → secondary → fallback text.
// "Steelers gold" stays gold (just deepened) instead of jumping to black.
function fgColor(team, bg = COLORS.surface) {
  if (!team) return COLORS.text;
  return fgFromHex(team.primary, bg) || fgFromHex(team.secondary, bg) || COLORS.text;
}

// Resolve which colors to display for a head-to-head matchup. If the two
// primaries are too similar (contrast < 2.5), swap one team to its secondary —
// pick whichever swap yields better contrast. Returns the colors to use plus
// the side that was swapped (so the UI can explain "shown in secondary").
function resolveMatchupColors(teamA, teamB) {
  if (!teamA || !teamB) {
    return {
      colorA: teamA ? teamA.primary : COLORS.blue,
      colorB: teamB ? teamB.primary : COLORS.gold,
      swappedSide: null, swappedTeam: null, swappedFromHex: null, swappedToHex: null,
    };
  }
  const aP = teamA.primary, bP = teamB.primary;
  if (contrastRatio(aP, bP) >= 2.5) {
    return { colorA: aP, colorB: bP, swappedSide: null, swappedTeam: null, swappedFromHex: null, swappedToHex: null };
  }
  const optA = teamA.secondary
    ? { colorA: teamA.secondary, colorB: bP, swappedSide: "A", swappedTeam: teamA, swappedFromHex: aP, swappedToHex: teamA.secondary, c: contrastRatio(teamA.secondary, bP) }
    : null;
  const optB = teamB.secondary
    ? { colorA: aP, colorB: teamB.secondary, swappedSide: "B", swappedTeam: teamB, swappedFromHex: bP, swappedToHex: teamB.secondary, c: contrastRatio(aP, teamB.secondary) }
    : null;
  const candidates = [optA, optB].filter(Boolean);
  ...
  return {
    colorA: best.colorA, colorB: best.colorB,
    swappedSide: best.swappedSide, swappedTeam: best.swappedTeam,
    swappedFromHex: best.swappedFromHex, swappedToHex: best.swappedToHex,
  };
}

// Friendly name for a hex (so we can say "shown in their gold" / "in orange").
const COLOR_NAMES = [
  ["red",     [255, 0, 0]],
  ["orange",  [255, 140, 0]],
  ["gold",    [212, 175, 55]],
...
function colorName(hex) {
  ...
  for (const [name, rgb] of COLOR_NAMES) {
    ...
    if (d < bestD) { bestD = d; best = [name, rgb]; }
  }
  return best[0];
}
...
function calculateMatchup(teamA, teamB, opts = {}) {
  ...
  // 12. Rushing attack — matchup of each team's rush yards/g vs the other
  // team's rush yards allowed. Driver matches the displayed numbers so
  // the comparison is transparent.
  {
    const aEdge = teamA.rushOffense.yardsPerGame - teamB.rushDefense.yardsAllowed;
    const bEdge = teamB.rushOffense.yardsPerGame - teamA.rushDefense.yardsAllowed;
    const delta = clamp((aEdge - bEdge) * 0.05, -3, 3);
    factors.push({
      ...FACTOR_DEFS[11],
      teamAValue: `${Math.round(teamA.rushOffense.yardsPerGame)} rush yd/g vs ${Math.round(teamB.rushDefense.yardsAllowed)} def`,
      teamBValue: `${Math.round(teamB.rushOffense.yardsPerGame)} rush yd/g vs ${Math.round(teamA.rushDefense.yardsAllowed)} def`,
      winner: winnerOf(delta), delta, magnitude: magnitudeOf(delta),
    });
  }

  // 13. Passing game — same matchup pattern with pass yards.
  {
    const aEdge = teamA.passOffense.yardsPerGame - teamB.passDefense.yardsAllowed;
    const bEdge = teamB.passOffense.yardsPerGame - teamA.passDefense.yardsAllowed;
    const delta = clamp((aEdge - bEdge) * 0.04, -4, 4);
    factors.push({
      ...FACTOR_DEFS[12],
      teamAValue: `${Math.round(teamA.passOffense.yardsPerGame)} pass yd/g vs ${Math.round(teamB.passDefense.yardsAllowed)} def`,
      teamBValue: `${Math.round(teamB.passOffense.yardsPerGame)} pass yd/g vs ${Math.round(teamA.passDefense.yardsAllowed)} def`,
      winner: winnerOf(delta), delta, magnitude: magnitudeOf(delta),
    });
  }
...
function TeamTile({ team, size = 56, color }) {
  if (!team) return null;
  const fill = color || team.primary;
  // White abbr on solid fill by default; if fill is too light, fall back to dark text.
  const onFill = contrastRatio("#ffffff", fill) >= 3.5 ? "#ffffff" : "#1f2328";
  // Trim border = the OTHER brand color (primary if we're displaying secondary, else secondary).
  const trim = color && color === team.secondary ? team.primary : team.secondary;
  return (
    <div
      aria-hidden="true"
      style={{
        ...
        border: `2px solid ${trim || COLORS.border}`,
...
// === Loading scanner ===
function ScanningPanel({ teamA, teamB, colorA, colorB }) {
  const messages = [
  ...
  return (
    ...
    >
      ...
      >
        <TeamTile team={teamA} color={colorA} size={48} />
        <span style={{ color: fgFromHex(colorA) || COLORS.text }}>{teamA.name}</span>
        <span style={{ color: COLORS.textSec, fontSize: 16, letterSpacing: 2, fontWeight: 700 }}>VS</span>
        <span style={{ color: fgFromHex(colorB) || COLORS.text }}>{teamB.name}</span>
        <TeamTile team={teamB} color={colorB} size={48} />
      </div>
      <div
        style={{
...
export default function NFLWinPredictor() {
  ...
  // Resolve display colors for the matchup (auto-swap to a secondary if the
  // two primaries are too similar). One source of truth used in every
  // matchup-scoped view (scanning, reveal, result).
  const matchupColors = (teamA && teamB)
    ? resolveMatchupColors(teamA, teamB)
    : { colorA: teamA ? teamA.primary : COLORS.blue,
        colorB: teamB ? teamB.primary : COLORS.gold,
        swappedSide: null, swappedTeam: null, swappedFromHex: null, swappedToHex: null };
    ...
    >
      ...
      <div style={{ padding: "20px 24px", maxWidth: 980, margin: "0 auto" }}>
        ...
        {phase === "scanning" && teamA && teamB && (
          <ScanningPanel
            teamA={teamA} teamB={teamB}
            colorA={matchupColors.colorA} colorB={matchupColors.colorB}
          />
        )}
...
function RevealScreen({ teamA, teamB, matchup, revealedCount, animatedProb, onSkip, colorA, colorB, swap }) {
  ...
  const teamAFg = fgFromHex(colorA) || COLORS.text;
  const teamBFg = fgFromHex(colorB) || COLORS.text;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        ...
        <ProbabilityBar
          probA={probA} colorA={colorA} colorB={colorB}
          teamAName={teamA.name} teamBName={teamB.name}
        />
        {swap && swap.swappedSide && swap.swappedTeam && (
          <div
            role="note"
            style={{
              fontSize: 13, color: COLORS.textSec, fontWeight: 500,
          ...
          >
            <span
              style={{
                display: "inline-flex", flexShrink: 0, gap: 3,
              }}
            >
              <span style={{ width: 12, height: 12, borderRadius: 2, background: swap.swappedFromHex, border: `1px solid ${COLORS.border}` }} />
              <ArrowRight size={12} style={{ color: COLORS.textMute }} />
              <span style={{ width: 12, height: 12, borderRadius: 2, background: swap.swappedToHex, border: `1px solid ${COLORS.border}` }} />
            </span>
            <span>
              <strong style={{ color: COLORS.text }}>{swap.swappedTeam.name}</strong>{" "}
              shown in their secondary {colorName(swap.swappedToHex)} because both teams play in {colorName(swap.swappedFromHex)}.
            </span>
          </div>
        )}
        <div
...
function ResultScreen({ teamA, teamB, matchup, reset, colorA, colorB, swap }) {
  const aWins = matchup.finalProbability >= 50;
  const winner = aWins ? teamA : teamB;
  const loser = aWins ? teamB : teamA;
  const winnerColor = aWins ? colorA : colorB;
  const loserColor = aWins ? colorB : colorA;
  const winProb = aWins ? matchup.finalProbability : matchup.teamBProbability;

  const [showAllFactors, setShowAllFactors] = useState(false);
  const allFactorsRef = useRef(null);
  useEffect(() => {
    if (showAllFactors && allFactorsRef.current && typeof allFactorsRef.current.scrollIntoView === "function") {
      allFactorsRef.current.scrollIntoView({ behavior: "smooth", block: "start" });
    }
  }, [showAllFactors]);
    ...
    >
      <div
        style={{
          ...
          backgroundImage: `radial-gradient(circle at 50% 0%, ${winnerColor}22 0%, transparent 60%)`,
        }}
      >
        <div
          style={{
        ...
        <TeamTile team={winner} color={winnerColor} size={72} />
        <div
          style={{
            fontSize: TYPE.probHero, fontWeight: 900,
            color: fgFromHex(winnerColor) || COLORS.text,
            fontFamily: "ui-monospace, monospace",
            fontVariantNumeric: "tabular-nums", lineHeight: 1,
            textShadow: `0 4px 24px ${winnerColor}55`,
          }}
        >
          {Math.round(winProb)}%
        </div>
      ...
      >
        <SummaryCard
          ...
          color={biggestEdge ? (biggestEdge.delta > 0 ? colorA : colorB) : COLORS.textMute}
        />
        <SummaryCard
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
        ...
        <button
          ...
          aria-controls="nfl-all-factors-panel"
          style={{
            flex: "1 1 220px",
            padding: "18px 28px", fontSize: 17, fontWeight: 800,
            background: showAllFactors
              ? COLORS.surface
              : `linear-gradient(135deg, ${COLORS.blue}, ${COLORS.gold})`,
            color: showAllFactors ? COLORS.text : "#ffffff",
            border: showAllFactors ? `2px solid ${COLORS.border}` : "none",
            borderRadius: 14, cursor: "pointer", minHeight: 60,
            display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
            boxShadow: showAllFactors ? "none" : `0 4px 16px ${COLORS.blue}33`,
          }}
        >
          {showAllFactors ? <ChevronDown size={20} style={{ transform: "rotate(180deg)" }} /> : <BarChart3 size={20} />}
          {showAllFactors ? "Hide factor details" : "Show all 18 factors"}
        </button>
      </div>
        ...
        >
          ...
          >
            <div
              style={{
                fontSize: TYPE.caps, fontWeight: 700, letterSpacing: 1,
                color: COLORS.blue, textTransform: "uppercase", marginBottom: 6,
                display: "flex", alignItems: "center", gap: 8,
              }}
            >
              <BarChart3 size={16} /> All 18 factors, in order
            </div>
            <div style={{ fontSize: TYPE.body, color: COLORS.textSec, lineHeight: 1.5 }}>
              Each factor moves the prediction up or down a little.
              Add them all up to see how the model got to{" "}
              <strong style={{ color: COLORS.text }}>{Math.round(winProb)}%</strong>.
            </div>
          </div>
          {matchup.factors.map((f, i) => (
            <FactorCard
              key={f.id}
              factor={f}
              teamA={teamA} teamB={teamB}
